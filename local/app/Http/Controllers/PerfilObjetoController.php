...
{
function index($id){
$objeto = DB::select("SELECT *FROM objeto WHERE (NOT EXISTS(SELECT * FROM perfilobjeto,perfil WHERE objeto.id=perfilobjeto.idObjeto AND perfil.id=perfilobjeto.idPerfil AND objeto.deleted_at IS NULL AND perfilobjeto.idPerfil=".$id."))"); // Objeto::lists('nombre','id');
